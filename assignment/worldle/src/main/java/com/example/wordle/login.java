    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        loginbtn.setOnClickListener(view -> {
            ...
            if (password.length() < 6) {
                passwd.setError("Minimum length is >=6.");
                return;
            }
            ...
            fAuth.signInWithEmailAndPassword(email, password).addOnCompleteListener(task -> {
                if (task.isSuccessful()) {
                    ...
                    //incorrect
                    Toast.makeText(login.this, "KARIBU!!" , Toast.LENGTH_SHORT).show();

                }
            });
            loginbtn.setOnClickListener(view1 -> startActivity(new Intent(getApplicationContext(), InitialActivity.class)));
